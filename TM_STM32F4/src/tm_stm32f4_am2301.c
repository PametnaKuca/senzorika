...
TM_AM2301_t TM_AM2301_INT_Read(TM_AM2301_Data_t* data) {
	...
	// Determination of exact temperature and humidity from the received 5 bytes (see DHT22 datasheet)
	
	data->temperature = ((float)d[2] * 256 + (float) d[3])/10; //exact float value of temp. example 27.3
	data->humidity = ((float)d[0] * 256 + (float) d[1])/10;		//exact float value of humidity example 54.4
	data->temperature = (int) data->temperature;
	data->humidity = (int) data->humidity;
	
	/* Negative temperature is not implemented because it is not tested and verified*/
	/* Below is the unverified code for negative temperature 
	
	if (d[2] & 0x80) {
		data->temperature = (float) -((d[2] & 0x7F) << 8 | d[3]);
	} 
